...
fetch('https://jsonplaceholder.typicode.com/todos/1/users')
      ...
      .then(users => {
         users.forEach( user => {
               ...
               userInfo.innerHTML = htmlFromJson(user.name, user.email, user.website);
               main.appendChild(userInfo);
            }
         );
         });

function htmlFromJson(name, email, website){
  let html = `<h2 class="nombre"> ${name}</h2>
  <h3 class="email"> ${email} </h3>
  <h3 class="website"><a href="${website}"> ${website} </a></h3>`;
